...
/**
 * Task ID format. Two shapes accepted:
 * - **Current**: `TASK-YYYY-MM-DD-XXXXXXXX` — 8-char suffix from a visually-unambiguous
 *   nanoid alphabet (no 0/O/1/I/l). Eliminates the cross-store collision the prior
 *   per-store 3-digit counter caused (TASK-2026-04-26-001 minted twice on 2026-04-26).
 * - **Legacy**: `TASK-YYYY-MM-DD-NNN` (with optional `-NN` sub-suffix) — accepted on
 *   read so existing on-disk tasks remain valid; not generated by `nextTaskId` anymore.
 */
export const TaskId = z.string().regex(
  /^TASK-\d{4}-\d{2}-\d{2}-(?:\d{3}(?:-\d{2})?|[23456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{8})$/,
  "Invalid task id"
);
