...
AOF's memory system organizes knowledge into three tiers — hot, warm, and cold — each with distinct freshness, relevance, and indexing characteristics. Agents retrieve memory from the appropriate tier based on what they need.

<Aside type="note">
The current implementation uses **rule-based tier management** with gated promotion. LLM-based summarization and pressure-driven tier compaction are planned for a future release.
</Aside>

## The Three Tiers

```
Hot  ──── Canonical, always-indexed core documentation
   │        Always fresh, small (<50KB total)
   │        Read frequently, written rarely
   │
Warm ──── Team-scoped operational knowledge
   │        Updated automatically from task patterns
   │        Aggregated runbooks, decisions, status
   │
Cold ──── Immutable historical record
...
- **Gated promotion** — warm content must pass a review before being promoted to hot
...
- **Aggregated** — content is assembled from completed task outputs using rule-based logic
...
Documents flow up the tiers through a gated review process:
...
Rule-based: extract patterns, decisions, status
        │
        ▼
...
**Current implementation:** Aggregation from cold → warm is rule-based (pattern matching over event logs). LLM-based summarization is a planned future enhancement.
