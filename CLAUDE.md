...
- **Conversation-access hook gate** (OpenClaw ≥ 2026.4.23): the OpenClaw plugin registry silently drops `agent_end` / `llm_input` / `llm_output` registrations from non-bundled plugins like AOF unless `plugins.entries.aof.hooks.allowConversationAccess=true` is set in `~/.openclaw/openclaw.json`. The schema rejected this field on 4.23 and started accepting it on 4.24 (#71221); on 4.23 the gate is unbypassable. Symptom: dispatch latency regression — AOF's `agent_end` hook drives `triggerPoll("agent_end")`, so without it dispatch falls back from "wake on agent turn end" to "wake on `pollIntervalMs` (30s default)". Detection: gateway log emits `typed hook "agent_end" blocked because non-bundled plugins must set plugins.entries.aof.hooks.allowConversationAccess=true (plugin=aof, ...)` at boot. AOF's plugin startup also logs a `subscribed to OpenClaw plugin hooks` line listing `conversationAccessGated: ["agent_end"]` and the required config — cross-reference against the gateway diagnostic. Fix by setting the config field and reloading the gateway (`launchctl kickstart -k "gui/$(id -u)/ai.openclaw.gateway"` plus the daemon, per the dual-restart rule above).
