...
- **D-07:** Session lifecycle events: **selective forwarding**. Four state-mutating hooks forward via IPC: `session_end` → `POST /v1/event/session-end`, `agent_end` → `POST /v1/event/agent-end`, `before_compaction` → `POST /v1/event/before-compaction`, `message_received` → `POST /v1/event/message-received`. High-frequency capture hooks (`before_tool_call`, `after_tool_call`, `message_sent`) continue to update the in-plugin `OpenClawToolInvocationContextStore` locally — they mutate no daemon-owned state.

  **A1 (2026-04-17):** message_received forwards after all — handleMessageReceived calls protocolRouter.route(), which mutates daemon-owned session routing state. Discovered during RESEARCH.md code inspection; D-07 amended.

  - Rationale: minimizes IPC chatter on a busy gateway for the 3 truly-local capture hooks; forwards the 4 hooks that mutate daemon-owned state (session/agent lifecycle, compaction, protocol routing); preserves the "daemon is single writer" invariant (all mutations still go through the daemon, the local cache is read-only from the daemon's perspective). The captured route is attached as a parameter when `aof_dispatch` is invoked via IPC, so the daemon never loses notification recipients.
