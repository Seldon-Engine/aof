...
- 49E-5: **Redirect ephemeral session captures to `:main` at dispatch time, not just at wake time.** Surfaced 2026-05-01 by the subscription-graveyard audit: 23 of 26 ghost subscriptions had `sessionKey: agent:X:cron:UUID` or `agent:X:subagent:UUID` — captured when a one-shot dispatcher (cron job, subagent) called `aof_dispatch` with `notifyOnCompletion`. The dispatcher session terminated when its run ended; the dispatched task often outlived it; the chat-send half can never resolve a platform for the ephemeral key (the wake-injection half already redirects to `:main` per de5b6bd, but the chat-send half does not). Fix at dispatch time: in `mergeDispatchNotificationRecipient` (`src/openclaw/dispatch-notification.ts`), detect `:cron:` / `:subagent:` segments in the captured sessionKey and rewrite to `agent:X:<mainKey>` BEFORE persisting the subscription. Same redirect logic that exists in `chat-delivery-poller.ts:redirectEphemeralSessionKey`, just hoisted to capture time so both halves benefit. Captures the original ephemeral sessionKey in a sidecar field for debugging, but the load-bearing routing target is `:main`.
- 49E-6: **Subscription `deadletter` terminal state.** Today `SubscriptionStatus = "active" | "delivered" | "failed" | "cancelled"` (`src/schemas/subscription.ts:24`) and the recovery-replay pass retries `active` (and would retry `failed`) forever. Observed ghost subscriptions had `deliveryAttempts` of 22-28 with the same "cannot resolve platform for delivery" failure reason on every attempt — recovery never gives up. Add a `deadletter` status that parallels the task-lifecycle `deadletter`: terminal, automatic-retries-exhausted, needs human review. Trigger: 5 consecutive failures with the same `failureReason` shape (or 5 of any reason combined with no successful delivery in N days). Recovery-replay skips `delivered` + `cancelled` + `deadletter`. Existing `failed` semantic stays (single-attempt non-terminal failure that gets retried).
- 49E-7: **Embedded-run error-propagation contract.** Surfaced 2026-05-02 by the spear-phishing re-dispatch attempt: OpenClaw's `runEmbeddedPiAgent` swallows the `incomplete turn detected: payloads=0` failure mode (model returns HTTP 200 + `stop_reason: "stop"` with zero content). The hard-error path (`isError=true` for overload/timeout/connection failures) correctly propagates back via `meta.error`, but the soft `payloads=0` path is logged in OpenClaw and never surfaces to the caller. AOF then sees `agent run completed` cleanly, the agent never called `aof_task_complete`, and the task transitions to `blocked` via completion-enforcement. The 30-minute cadence in production logs (16+ occurrences in 8 hours, matching the AOF scheduler poll interval) means **most/all dispatched embedded-agent runs were silently failing** while we believed the system was healthy. **Fix surface:** (a) escalate upstream OpenClaw fix to propagate `payloads=0` as a `meta.error`; (b) AOF defensive heuristic — a short clean-meta run is almost certainly a silent model failure → `errorClass: "model_silent_failure"`, deadletter on first occurrence. Companion debug doc: `.planning/debug/2026-05-02-embedded-run-empty-response-and-error-propagation.md`. Companion LADR: 0015-embedded-run-error-contract.md.

  **Partial implementation status (commits landing 2026-05-02):** AOF-side defensive heuristic shipped (commit `76e8ea6`) — `isLikelyModelSilentFailure(outcome)` in `src/dispatch/scheduler-helpers.ts` (returns true when `outcome.error` undefined + `outcome.aborted` false + `outcome.durationMs < 60_000`), wired into `handleRunComplete`'s enforcement branch, classified as `errorClass: "model_silent_failure"` and short-circuited in `shouldTransitionToDeadletter` parallel to the existing `"permanent"` path. Regression test: `src/dispatch/__tests__/bug-2026-05-02-embedded-run-silent-failure-detection.test.ts`.

  **Investigation instrumentation shipped (separate commit, same day):** dedicated `silent_model_failure` event type emitted from `handleRunComplete`, `aof_silent_model_failures_total` Prometheus counter registered (not yet incremented — see TODO below), cross-reference investigation pointers (`expectedSessionFile`, `grepHint`, `openclawErrLog` path) attached to the structured log line + task metadata + event payload. Each piece marked with `INSTRUMENTATION-2026-05-02` source comments. Debug recipe documenting how to investigate next-occurrence: `.planning/debug/2026-05-02-embedded-run-empty-response-and-error-propagation.md`.

  **What's still pending in 49E-7:**
  1. **Upstream OpenClaw fix** so `meta.error` carries the `payloads=0` failure info — heuristic is a guard, not a true fix.
  2. **Tighter heuristic signal** beyond duration alone (count `aof_task_*` tool invocations via EventLogger lookup and require zero before classifying as silent-failure — would let us shorten the threshold and reduce false-positive risk for legitimately-fast tasks).
  3. **Wire the Prometheus counter increment** — counter is registered but `handleRunComplete` doesn't yet receive `metrics`. Needs threading through `OnRunCompleteContext` (signature change cascades to `executeAssignAction → handleAssign → executeActions`). Until then, frequency is queryable via the event log.
  4. **Cleanup the temporary instrumentation when investigation closes.** Authoritative removal/retention plan: see "CLEANUP CHECKLIST" section in `.planning/debug/2026-05-02-embedded-run-empty-response-and-error-propagation.md`. Summary: REMOVE the verbose investigation pointers + per-task metadata stamping + recipe doc; KEEP the event type + counter + heuristic + regression test as long-term defensive monitoring infrastructure. Trigger condition for cleanup: upstream OpenClaw fix lands AND zero `silent_model_failure` events for ≥7 consecutive days. Track every `INSTRUMENTATION-2026-05-02` comment in source for the full inventory.
...
| 0013 | redirect-ephemeral-sessionkey-at-capture | 49E-5 |
| 0014 | subscription-deadletter-terminal-state | 49E-6 |
| 0015 | embedded-run-error-contract | 49E-7 |
