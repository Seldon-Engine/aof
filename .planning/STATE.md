...
- Phase 47 added: Validate and stabilize the user upgrade pipeline — settle on one canonical upgrade path (likely `aof update`, with `install.sh` re-run as fallback), make it work end-to-end across version transitions, ship CI test automation that exercises real upgrade scenarios. Triggered post-v1.18.1 when removing a "neither path has been exercised in recent releases" disclaimer from README revealed the gap. Note: Phase 46's roadmap entry mentions Tier B bugs being "held for Phase 47" — that referent is now mismatched (this Phase 47 is upgrade-pipeline, not Tier B); Tier B should land as a separate phase or 999.x backlog item.
